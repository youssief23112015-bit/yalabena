...
export class PlacementTestsService {
  ...
  async generateWrittenPaper(
    ...
    if (test.status === TestStatus.COMPLETED) {
      throw new BadRequestException(
        'Cannot generate a paper for a completed test',
      );
    }
    ...
    if (!bank.length) {
      throw new BadRequestException(
        `No active questions found for level ${opts.level}`,
      );
    }
  ...
  /**
   * Auto-scores a written submission.
   *
   * Objective types (MCQ, TRUE_FALSE, FILL_BLANK, MATCHING, ORDERING)
   * are graded automatically.
   *
   * SHORT_ANSWER is stored with score 0 and is_correct = null
   * for manual grading.
   *
   * A written test is SINGLE-ATTEMPT:
   * once written_score has been recorded, another submission is rejected
   * with HTTP 409 Conflict.
   *
   * The delete + insert + score update run in ONE transaction,
  ...
  async submitWritten(testId: string, dto: SubmitWrittenDto) {
     ...
     *
     * written_score is null before the first submission.
     * After a successful submission it contains the calculated score.
     *
     * This prevents a student from submitting the same placement test
     * repeatedly and inspecting isCorrect values from the response.
     */
    ...
    const questions = await this.questionRepo.find({
      where: {
        id: In(items.map((a) => a.question_id)),
      },
    });
    ...
    if (unknownIds.length > 0) {
      throw new BadRequestException(
        `Unknown question(s): ${unknownIds.join(', ')}`,
      );
    }
    ...
    for (const item of items) {
      ...
      const { isCorrect, score } = this.scoreQuestion(
        question,
        item.answer,
      );
    ...
    const writtenScore =
      possible > 0
        ? Math.min(
            ...
            Math.max(
              0,
              Math.round((earned / possible) * scale * 100) / 100,
            ),
          )
        : 0;

    await this.repo.manager.transaction(async (manager) => {
      await manager.delete(PlacementTestAnswer, {
        test_id: testId,
      });
  ...
  private scoreQuestion(
    ...
    switch (question.type) {
      ...
      case QuestionType.FILL_BLANK: {
        ...
        const accepted: string[] = Array.isArray(correct)
          ? correct.map(String)
          : [String(correct)];

        const ok = accepted.some(
          (c) => norm(c) === norm(answer),
        );
  ...
  private shuffle<T>(arr: T[], seed?: number): T[] {
    ...
    // xorshift32 gets stuck at 0,
    // so never start from 0.
