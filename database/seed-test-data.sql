...
-- test_questions (A1 written bank)
-- The original seed question had no options and correct_answer "A". MCQ answers are
-- stored as the option TEXT (what the frontend submits).
...
-- More A1 questions so the test can actually tell students apart.
...
-- NOTE: no placement_test_answers rows are seeded. A pre-filled answer for the
-- (not yet taken) placement test collided with the first real submit on
-- uq_pta_test_question and made "already submitted" ambiguous.
